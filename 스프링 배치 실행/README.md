...
### 2. `스프링 배치 스코프`
  - `@JobScope, @StepScope`
    ...
    - 해당 스코프가 선언되면 빈이 생성이 애플리케이션 구동시점이 아닌 빈의 실행시점에 이루어진다
      - @Values를 주입해서 빈의 실행 시점에 값을 참조할 수 있으며 일종의 LazyBinding이 가능해진다
      - @Value("#{jobParameters[파라미터명]}"),@Value("#{jobExecutionContext[파라미터명]}"),@Value("#{stepExecutionContext[파라미터명]}")
    - 프록시 모드로 빈이 선언되기 때문에 애플리케이션 구동시점에는 빈의 프록시 객체가 생성되어 실행 시점에 실제 빈을 호출해준다
...
![image](https://user-images.githubusercontent.com/40031858/160265829-67cc524e-8c91-407a-9376-0a6a999457fa.png)
...
- Job실행 시 Proxy 객체가 실제 빈을 호출해서 해당 메소드를 실행시키는 구조
...
![image](https://user-images.githubusercontent.com/40031858/160266756-cd47018a-3db1-4462-9c7e-e24968e44de9.png)
